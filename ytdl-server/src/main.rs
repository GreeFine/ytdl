...
// async fn router(req: Request<Body>) -> BoxFuture {
//     let response;
//     let internal_server_error = |error: Error| {
//         let json =
//             serde_json::to_string_pretty(&json!({ "error": format!("{}", error.as_fail()) }))
//                 .unwrap();
//         Response::builder()
...
//         (&Method::GET, "/watch") => {
//             response = get_video(&req).await.unwrap_or_else(internal_server_error);
//         }
...
// async fn get_video(req: &Request<Body>) -> Result<Response<Body>> {
//     let query = req
//         .uri()
//         .query()
...
async fn main() -> Result<()> {
    let query = "v=GttAu27FY58";
    let hash_query: HashMap<String, String> = form_urlencoded::parse(query.as_bytes())
        .into_owned()
        .collect();
    println!("QUERY: {:?}", hash_query);
    let video_id = validate_query(&hash_query, "v")?;
    let mut video = Video::new(&video_id);
    let _ = video.initialize().await?;
    println!("after init?");
    let video_sources = video
        .video_sources()
        .ok_or_else(|| err_msg("Ops, Error While Getting Video Sources."))?;
